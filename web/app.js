...
const App = {
    ...
    inferenceSubSkills: [],
    inferenceActiveSubSkill: '',
    inferenceDoneSubSkills: [],
    ...
    rerenderDynamic() {
        try {
            ...
            if (this.orchPreview)
                this.renderOrchPreview(this.orchPreview);
            if (this.orchStatusKey?.params) {
                const el = document.getElementById('orch-status-indicator');
                if (el)
                    el.textContent = this.t(this.orchStatusKey.key, this.orchStatusKey.params);
            }
    ...
    handleEvent(msg) {
        ...
        switch (event) {
            ...
            case 'training_started':
                ...
                this.renderTrainingSkillsTree();
                this.updateTrainingStatus(this.t('status.trainingStep', { s: data }), 'var(--cyan)', 'status.trainingStep', { s: data });
                break;
            ...
            case 'training_finished':
                ...
                this.updateTrainingStatus(this.t('status.trainDoneCkpt'), 'var(--green)', 'status.trainDoneCkpt');
                if (data.skill) {
            ...
            case 'training_error':
                ...
                this.updateTrainingStatus(this.t('status.trainErrShort', { e: data.error }), 'var(--red)', 'status.trainErrShort', { e: data.error });
                if (data.skill) {
                    const txt = document.getElementById(`train-progress-text-${data.skill}`);
                    if (txt)
                        txt.textContent = this.t('val.error');
                }
                if (this.activeTrainingSkill === data.skill) {
                    this.activeTrainingSkill = null;
                }
            ...
            case 'orchestration_task_started':
                ...
                this.updateOrchStatus(this.t('orch.activeStep', { task: data }), 'var(--yellow)', 'orch.activeStep', { task: data });
            ...
            case 'orchestration_finished':
                this.updateOrchStatus(this.t(data ? 'orch.finishedOk' : 'orch.finishedErr'), data ? 'var(--green)' : 'var(--yellow)', data ? 'orch.finishedOk' : 'orch.finishedErr');
            ...
            case 'orchestration_error':
                this.updateOrchStatus(this.t('orch.error', { msg: String(data) }), 'var(--red)', 'orch.error', { msg: String(data) });
                break;
            case 'pong':
                break;
        }
        ...
        for (const side of ['leader', 'follower']) {
            ...
            if (btnSetup && details) {
                if (calibrated) {
                    ...
                    if (stateEl)
                        stateEl.textContent = this.t('cal.stateCalibrated');
                }
                else {
    ...
    async loadSysInfo() {
        ...
        try {
            ...
            if (sysinfo) {
                // Pre-fill paths
                const projDirInput = document.getElementById('settings-projects-dir');
                if (projDirInput)
                    projDirInput.value = sysinfo.projects_dir || '';
    ...
    updateArmStatusCards() {
        ...
        if (btnLeader && detailsLeader) {
            if (leaderPort) {
                ...
                if (leaderPortText)
                    leaderPortText.textContent = leaderPort;
                if (leaderTypeText)
        ...
        if (btnFollower && detailsFollower) {
            if (followerPort) {
                ...
                if (followerPortText)
                    followerPortText.textContent = followerPort;
                if (followerTypeText)
    ...
    hwOnSelectConfiguredCamera(camId) {
        const cam = (this.project?.cameras || []).find((c) => c.id === camId);
        if (!cam)
            return;
        const portSelect = document.getElementById('hw-camera-port-select');
        ...
        if (portSelect && cam.source !== undefined) {
            ...
            let opt = portSelect.querySelector(`option[value="${sourceStr}"]`);
            if (!opt) {
                opt = document.createElement('option');
                opt.value = sourceStr;
    ...
    async calibrateArm(role) {
        ...
        const btn = (document.getElementById(`cal-btn-setup-${role}`) || document.getElementById(`btn-calibrate-${role}`));
    ...
    renderCalibrationStoredRanges(arm) {
        ...
        if (src) {
            ...
            if (filename)
                src.removeAttribute('data-i18n');
            else
                src.setAttribute('data-i18n', key);
    ...
    async startWorkflowTraining() {
        ...
        this.log('INFO', this.t('log.trainQueueStart', { s: checkedSkills.join(', '), steps, batch: batchSize }));
        this.renderTrainingSkillsTree();
        ...
        if (btn) {
            btn.disabled = true;
            btn.textContent = this.t('btn.startingShort');
        }
        try {
            ...
            if (btn) {
                btn.disabled = false;
                btn.textContent = label || this.t('btn.startTraining');
            }
        }
    },
    ...
    async startWorkflowInference() {
        ...
        try {
            ...
            if (btn) {
                btn.textContent = label || this.t('btn.deployPolicy');
            }
            this.updateActionButtonStates();
        }
        if (res && res.ok !== false) {
    ...
    onRobotTypeChange() {
        ...
        if (leaderTypeInput)
            leaderTypeInput.value = (entry?.leader_type) || 'so100_leader';
        if (followerTypeInput)
            followerTypeInput.value = (entry?.robot_type) || robotType;
        this.renderRobotTypeList();
        this.updateConnectCmdPreview();
        this.updateHardwareButtonStates();
    },
    ...
    deviceTypeFor(name) {
        ...
        if (!list.length)
            return null;
        const hit = list.find((d) => d.robot_type === name);
        if (hit)
            return hit;
        const asLeader = list.find((d) => d.leader_type === name);
        if (asLeader)
            return asLeader;
        // Names older versions wrote: "so100", "lekiwi_follower", "reachy2_leader"
        const base = name.replace(/_(follower|leader)$/, '');
        return list.find((d) => d.robot_type === base)
            || list.find((d) => d.robot_type === `${base}_follower`)
            || null;
    },
    ...
    syncRobotTypeOptions() {
        ...
        const signature = list.map((d) => d.robot_type).join(',');
        if (select.getAttribute('data-signature') !== signature) {
            select.innerHTML = list.map((d) => `<option value="${this.esc(d.robot_type)}">${this.esc(d.robot_type)}</option>`).join('');
    ...
    renderRobotTypeList() {
        ...
        this.syncRobotTypeOptions();
        const active = document.getElementById('robot-type-select')?.value || '';
        const usable = list.filter((d) => d.supported).length;
        if (count)
            count.textContent = this.t('lbl.nOfMDrivable', { n: usable, m: list.length });
    ...
    updateConnectCmdPreview() {
        ...
        const leaderId = document.getElementById('tele-leader-id')?.value || 'my_leader_arm';
        const followerId = document.getElementById('tele-follower-id')?.value || 'my_follower_arm';
        const hasLeader = entry ? entry.has_leader : true;
        if (state) {
    ...
    renderDetectedHardware() {
        ...
        const leaderPort = this.resolvedArmPort('leader');
        const followerPort = this.resolvedArmPort('follower');
        const projectCams = this.project?.cameras || [];
        ...
        const entry = this.deviceTypeFor(document.getElementById('robot-type-select')?.value);
        const need = (entry ? entry.has_leader : true) ? 2 : 1;
        const have = (need === 2 && leaderPort ? 1 : 0) + (followerPort ? 1 : 0);
        if (numPorts)
            numPorts.textContent = String(ports.length);
        if (numCams)
            numCams.textContent = String(cams.length);
        if (numAssigned)
            numAssigned.textContent = `${have}/${need}`;
        if (numCamsUsed)
            numCamsUsed.textContent = String(projectCams.length);
        ...
        if (portsHost) {
            ...
            <th>${this.esc(this.t('th.role'))}</th>
                    ...
                    ports.map((p) => {
                        ...
                        if (p.device === leaderPort) {
                            role = this.t('lbl.leaderShort');
                            roleCls = 'conn-role-used';
                        }
                        else if (p.device === followerPort) {
                            role = this.t('lbl.followerShort');
                            roleCls = 'conn-role-used';
                        }
        ...
        if (camsHost) {
            ...
            <th>${this.esc(this.t('th.role'))}</th>
                ...
                <td><span class="${used ? 'conn-role-used' : 'conn-role-free'}">${this.esc(used ? this.t('lbl.inProject') : '–')}</span></td>
              </tr>`;
                    }).join('') + '</tbody></table>';
            }
        }
    ...
    async manualRescanHardware() {
        const btn = document.getElementById('btn-rescan-hardware');
        const old = btn ? btn.textContent : '';
        if (btn) {
            btn.disabled = true;
            btn.textContent = this.t('btn.rescanning');
        }
        try {
            await this.scanHardware();
        }
            ...
            if (btn) {
                btn.disabled = false;
                btn.textContent = old || this.t('btn.rescanHw');
            }
        }
    },
    loadModelConfig() {
    ...
    async executeOrchestration() {
        const inputEl = document.getElementById('orch-input');
        ...
        if (btn) {
            btn.disabled = true;
            btn.textContent = this.t('btn.runPlanBusy');
        }
        try {
            ...
            if (btn) {
                btn.disabled = false;
                btn.textContent = label || this.t('btn.runPlan');
            }
        }
    },
    ...
    async loadOrchPreview() {
        const btn = document.getElementById('btn-orch-preview-refresh');
        const label = btn?.textContent || '';
        // Every step is a filesystem check on the server, so this is not instant
        // on a project with many steps.
        if (btn) {
            btn.disabled = true;
            btn.textContent = this.t('val.loadingShort');
        }
        try {
    ...
    renderOrchPreview(data) {
        ...
        list.innerHTML = tasks.map(task => {
            const stepRows = (task.steps || []).map((step, i) => `
        <div class="orch-step">
          ...
          <span class="orch-step-body">
            ...
            <span class="orch-step-path" title="${this.esc(step.policy_path)}">${this.esc(String(step.policy_path).split(/[\\/]/).filter(Boolean).pop() || step.policy_path)}</span>
          </span>
          <span class="orch-step-flag ${step.policy_ready ? 'ready' : 'missing'}">${this.esc(this.t(step.policy_ready ? 'val.ckptReady' : 'val.ckptMissing'))}</span>
        </div>`).join('');
            return `
        <div class="orch-task ${task.orchestrated ? 'is-orchestrated' : ''}">
          <div class="orch-task-head">
            ...
            <span class="pd-task-mode ${task.orchestrated ? 'ok' : 'baseline'}">${this.esc(this.t(task.orchestrated ? 'val.modeBoth' : 'val.modeBaseline'))}</span>
          </div>
    ...
    updateOrchStatus(message, color, key, params) {
        const el = document.getElementById('orch-status-indicator');
        if (!el)
            return;
    ...
    async loadTrainingTargets() {
        const btn = document.getElementById('btn-train-reload');
        const label = btn?.textContent || '';
        if (btn) {
            btn.disabled = true;
            btn.textContent = this.t('btn.loading');
    ...
    renderTrainingSkillsTree() {
        ...
        const tasks = data?.tasks || [];
        const all = tasks.flatMap(t => [t.baseline, ...(t.steps || [])]).filter(Boolean);
        const setStat = (id, text, warn = false) => {
            const el = document.getElementById(id);
            if (!el)
                return;
        ...
        const row = (target, opts) => {
          ...
          <div class="train-row-progress">
            ...
            <div class="train-row-progress-meta">
              <span id="train-progress-text-${this.esc(slug)}">${this.esc(this.t(active ? 'val.training' : 'val.queued'))}</span>
        ...
        <div class="${cls}" title="${this.esc(blockedTitle)}">
          ...
          <span class="train-row-flags">
            <span class="train-flag ${target.dataset_ready ? 'ready' : 'missing'}">${this.esc(this.t(target.dataset_ready ? 'val.dataOk' : 'val.dataMissing'))}</span>
            <span class="train-flag ${target.policy_ready ? 'ready' : 'missing'}">${this.esc(this.t(target.policy_ready ? 'val.ckptReady' : 'val.ckptMissing'))}</span>
          </span>
          ${progress}
        </div>`;
        };
    ...
    selectedTrainTargets() {
        return (this.trainTargets?.tasks || [])
            .flatMap((t) => [t.baseline, ...(t.steps || [])])
            .filter((x) => x && x.dataset_ready && this.trainSelected.has(x.slug));
    },
    /** Records a row's tick in state so a re-render keeps it. */
    setTrainTargetChecked(slug, checked) {
        if (checked)
            this.trainSelected.add(slug);
        else
            this.trainSelected.delete(slug);
        this.updateTrainCmdPreview();
    },
    ...
    updateTrainCmdPreview() {
        ...
        const steps = document.getElementById('train-steps')?.value || '10000';
        const batch = document.getElementById('train-batch-size')?.value || '8';
        el.textContent = targets.map(x => `lerobot-train --policy.type=${arch} --dataset.repo_id=${x.repo_id} --steps=${steps} --batch_size=${batch}`).join('\n');
    },
    ...
    toggleTrainParentCheckbox(parent, checked) {
        const task = (this.trainTargets?.tasks || []).find((t) => t.slug === parent);
        if (!task)
            return;
        [task.baseline, ...(task.steps || [])].filter(Boolean).forEach((target) => {
            if (!target.dataset_ready)
                return; // disabled: the trainer would refuse it
            const cb = document.getElementById(`train-check-${target.slug}`);
            if (cb)
                cb.checked = checked;
    ...
    updateTrainingStatus(text, color, key, params) {
    ...
    updateTrainingProgress(step, loss, skill_slug) {
        ...
        this.updateTrainingStatus(this.t('status.trainStepLoss', { step, loss: loss.toFixed(5) }), 'var(--yellow)', 'status.trainStepLoss', { step, loss: loss.toFixed(5) });
        ...
        if (activeSkill) {
            ...
            if (txt)
                txt.textContent = this.t('val.stepOfTotal', { step, total: totalSteps });
    ...
    setInferencePanelVisible(visible) {
    ...
    initPersistentInferenceUI() {
        ...
        if (deltaEl)
            deltaEl.textContent = '0.0000 rad';
    ...
    renderInferenceSubtasks(subSkills) {
        ...
        <div class="mr-subtask ${state}" id="infer-row-${this.esc(sub)}">
          ...
          <button class="btn btn-xs ${running || done ? 'btn-success' : 'btn-primary'}"
                  ...
                  title="${this.esc(this.t('tip.dispatchSubtask'))}" ${running ? 'disabled' : ''}>${this.esc(this.t(btnKey))}</button>
        </div>`;
        }).join('');
    },
    ...
    setInferenceSubtaskRunning(subSkill) {
    ...
    handleInferenceTelemetry(line) {
        ...
        if (line.includes('[STATUS] TASK_DONE')) {
            ...
            if (doneMatch) {
                ...
                if (this.inferenceActiveSubSkill === subSkill)
                    this.inferenceActiveSubSkill = '';
                this.renderInferenceSubtasks(this.inferenceSubSkills);
                this.updateInferenceDaemonStatus('WAITING');
