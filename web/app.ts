...
const App = {
  ...
  // user typed by hand.
  slugState: {} as Record<string, { valid: boolean; touched: boolean; timer: number }>,
  ...
  async createProject(): Promise<void> {
    ...
    const btn = document.getElementById('new-project-submit-btn') as HTMLButtonElement | null;
    ...
    if (btn) { btn.disabled = true; btn.textContent = this.t('btn.creating'); }
    let r: any;
    try {
  ...
  openNewProjectChoiceModal(): void {
    this.openModal('modal-create-project-choice');
  },

  openArmPortSetupModal(arm: 'leader' | 'follower'): void {
    ...
    if (sel) {
      ...
      const curVal = arm === 'leader'
        ...
        : (document.getElementById('tele-follower-port') as HTMLSelectElement | null)?.value;
  ...
  saveArmPortFromModal(): void {
    const sel = document.getElementById('modal-arm-port-select') as HTMLSelectElement | null;
    const port = sel?.value || '';
    const arm = this.autoDetectActiveArm;
    if (arm === 'leader') {
      const leaderSel = document.getElementById('tele-leader-port') as HTMLSelectElement | null;
      if (leaderSel) {
      ...
      const followerSel = document.getElementById('tele-follower-port') as HTMLSelectElement | null;
      if (followerSel) {
  ...
  showSkillWizardStep2(): void {
    ...
    const slugInput = document.getElementById('new-skill-slug') as HTMLInputElement | null;
    if (slugInput) slugInput.readOnly = this.skillWizardIsEdit;
  ...
  async submitSkillWizard(): Promise<void> {
    ...
    if (this.skillWizardIsEdit) {
      ...
      // CREATE MODE
      if (!slug && name) slug = this.slugify(name);
      if (!slug) {
   ...
   */
  bindAutoSlug(): void {
    const pairs: Array<{ nameId: string; slugId: string; kind: 'project' | 'skill' }> = [
  ...
  resetSlugState(slugId: string): void {
   ...
   */
  checkSlug(slugId: string, kind: 'project' | 'skill'): void {
  ...
  async runSlugCheck(slugId: string, kind: 'project' | 'skill'): Promise<void> {
    const slugEl = document.getElementById(slugId) as HTMLInputElement | null;
    const statusEl = document.getElementById(`${slugId}-status`);
    if (!slugEl) return;
    const slug = slugEl.value.trim();
    const nameEl = document.getElementById(
      slugId === 'new-project-slug' ? 'new-project-name' : 'new-skill-name') as HTMLInputElement | null;
    ...
    if (!slug) {
      ...
      if (statusEl) { statusEl.textContent = ''; statusEl.className = 'slug-status'; }
      return;
    }

    const body: Record<string, string> = { slug, kind, name: nameEl?.value || '' };
    if (kind === 'skill') {
      body.parent_slug = this.skillWizardType === 'step'
        ? ((document.getElementById('new-skill-parent') as HTMLSelectElement | null)?.value || '')
        : '';
    } else {
      body.parent_dir = (document.getElementById('new-project-parent-dir') as HTMLInputElement | null)?.value.trim() || '';
    }
    ...
    if (slugEl.value.trim() !== slug) return;
  ...
  updateSlugSubmitState(slugId: string): void {
    ...
    if (slugId === 'new-skill-slug' && this.skillWizardIsEdit) { btn.disabled = false; return; }
